...
filename = "ILtoAddCount"+str(today)
...
for row in range(rows):
    ...
    if text_from_table == 'LaSalle County':
    ...
    pyautogui.press('left',presses=6, interval=0.03)
    pyautogui.hotkey('command', 'c')
    pyautogui.hotkey('command', 'c')
    pyautogui.keyDown('command')
    pyautogui.keyUp('command')
    ...
    pyautogui.press('right',presses=6, interval=0.03)   
    time.sleep(.5)
    ...
    if (
        len(muni_list)>1 and text_from_table not in muni_list[0]
        ):
